...
@section('content')
    <section class="section">
        ...
        <form action="{{ route('front.community.productions.update', ['id' => $production->id]) }}" method="POST">
            @csrf
            @method('put')
            <div class="card">
                        ...
                        <div class="col-lg-6 col-md-6 col-sm-6 col-12">
                            <div class="form-group">
                                ...
                                <select class="form-control @error('commodity') is-invalid @enderror selectric" id="commodity" name="commodity">
                                    ...
                                    <option value="{{ $commodity->id }}" {{ old('commodity') ?? $production->commodity_id == $commodity->id ? 'selected' : '' }}>{{ $commodity->name }}</option>
                                    @endforeach
                                </select>
                                @error('commodity')
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="year_production" id="year_production" class="form-control @error('year_production') is-invalid @enderror" value="{{ old('year_production') ?? $production->year_production}}" placeholder="Tahun Produksi" required>
                            ...
                            <div class="form-group">
                                ...
                                <select class="form-control @error('quartal') is-invalid @enderror selectric" id="quartal" name="quartal">
                                    ...
                                    <option value="Q3" {{ old('quartal') ?? $production->quartal == 'Q3' ? 'selected' : '' }}>Kuartal 3</option>
                                    <option value="Q4" {{ old('quartal') ?? $production->quartal == 'Q4' ? 'selected' : '' }}>Kuartal 4</option>
                                </select>
                                @error('quartal')
                                    <div class="invalid-feedback" role="alert">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="stock" id="stock" class="form-control @error('stock') is-invalid @enderror" value="{{ old('stock') ?? $production->stock }}" placeholder="Stok" required>
                                @error('stock')
                                    <div class="invalid-feedback" role="alert">
                ...
                <div class="card-footer text-center">
                    <input type="submit" value="Ubah" class="btn btn-success btn-text">
                </div>
            </div>
        </form>
    </section>
